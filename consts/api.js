const getPostPositionPrompt = (withStructure) => {
  ...
  if (withStructure) {
    return `${basePrompt} 추가적으로 현재 내용 기준으로 유사도 없는 다른 창의적인 구조로 글을 하나 만들어주는 데, 조건은 기존 핵심 키워드 및 내용은 변하지 않게 해줘. 그리고 모바일에서도 해당 글을 읽을 수 있기 때문에 가독성 높은 구조로 각 문장의 길이가 너무 크지 않게 줄여줘.`;
  }
  return basePrompt;
};
